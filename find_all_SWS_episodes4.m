function [t_mdpt_SWS,data_at_SWS_midpoints,t_mdpt_indices]=find_all_SWS_episodes4(datafile,timestampvec,epoch_length)
%USAGE:
% [t_mdpt_SWS,t_midpt_indices,data_at_SWS_midpoints]=find_all_SWS_episodes4(datafile)
% 
...
%
%
% NOTES: this is like find_all_SWS_episodes3.m, except it uses overlapping sliding 
% windows of 5 minutes rather than adjacent windows.  It works, but the output 
% is rather noisy, so I'm sticking with find_all_SWS_episodes3.m.  
...
num_windows_in_recording = floor(length(data)-num_epochs_in_window+1);  % now using sliding windows, not adjacent windows
...
d=diff(timestampvec);
 ...
 for i=1:num_windows_in_recording
  if max(d(i:i+num_epochs_in_window-2)) == seconds(epoch_length)
...
for i=non_missing_data_windows
  if length(find(sleepstate(i:i+num_epochs_in_window-1)==1))>=percentage*num_epochs_in_window
    t_mdpt_SWS(SWS_episode_index) = timestampvec(i)+seconds(epoch_length*num_epochs_in_window/2);
    data_at_SWS_midpoints(SWS_episode_index) = median(data(i:i+num_epochs_in_window-1));
    t_mdpt_indices(SWS_episode_index)=round(mean([i,i+num_epochs_in_window-1]));  % these are indices after removing artifacts
